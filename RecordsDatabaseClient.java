...
public class RecordsDatabaseClient extends Application {
    ...
    public void requestService() {
        try {
			...
			OutputStreamWriter requestStreamWriter = new OutputStreamWriter(this.clientSocket.getOutputStream());
    ...
    public void reportServiceOutcome() {
        try {
			...
			serviceOutcome = (CachedRowSet) objectInputStream.readObject();

			GridPane grid = (GridPane) thePrimaryStage.getScene().getRoot();
			TableView<MyTableRecord> result = (TableView<MyTableRecord>) grid.getChildren().get(5);

			ObservableList<MyTableRecord> tmpRecords = result.getItems();
			tmpRecords.clear();

			while(this.serviceOutcome.next()) {
				MyTableRecord temp = new MyTableRecord();

				temp.setTitle(this.serviceOutcome.getObject("title").toString());
				temp.setLabel(this.serviceOutcome.getObject("label").toString());
				temp.setGenre(this.serviceOutcome.getObject("name").toString());
				temp.setRrp(this.serviceOutcome.getObject("rrp").toString());
				temp.setCopyID(this.serviceOutcome.getObject("copyid").toString());
			...
			for(MyTableRecord row : tmpRecords){
				System.out.println(row.getTitle() + " | " + row.getLabel() + " | " + row.getGenre() + " | " + row.getRrp() + " | " + row.getCopyID());
			}
    ...
    public void execute(){
		...
		this.userCommand = artistSurname + ";" + recordShopCity;
